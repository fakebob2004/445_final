...
\begin{figure}[htb!]
    ...
    \fbox{\parbox{0.9\linewidth}{\centering
        ...
        \vspace{1.8cm}}}
    \caption{Voice-controlled FPV drone system architecture (ground unit, RF link, and
        airframe).}
...
module via UART.
The RP2, reflashed with ELRS transmitter firmware, broadcasts RC frames at 50~Hz over the
2.4~GHz ISM band.
On the aircraft, a RadioMaster RX24T ELRS receiver forwards decoded RC channel data over
CRSF to the SpeedyBee F405~V5 flight controller running Betaflight.
The flight controller drives four 1860~KV brushless motors through an OX32 55~A four-in-one
ESC.
Barometric altitude telemetry from the flight controller is relayed back through the same RF link
and decoded by the ESP32, then displayed on the OLED for the ground operator.
